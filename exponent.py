...
class Example(Scene):
    def construct(self):
        ...
        exp_graph=axes.plot(lambda x: np.exp(x)-1, color=BLUE, stroke_opacity=0.2)
        ...
        exp_zoom=axes_zoom.plot(lambda x: np.exp(x)-1, color=BLUE, stroke_opacity=0.2)
        ...
        text= Text("exp(x)~x при x->0", font_size=48)
